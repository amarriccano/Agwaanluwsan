<template>
  <div>
    <div class="page-header">
      <h1>Articles</h1>
      <p>Peer-reviewed academic articles and scholarly contributions. Most recent first.</p>
    </div>
...
useHead({ title: 'Articles — Batsaikhan Ookhnoi' })
const api = useApi()
const loading = ref(true)
const items = ref<any[]>([])

onMounted(async () => {
  try { items.value = await api.get<any[]>('/content?type=article') }
