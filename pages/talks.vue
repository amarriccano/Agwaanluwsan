<template>
  <div>
    <div class="page-header">
      <h1>Talks</h1>
      <p>Keynote addresses, conference papers, and invited lectures. Most recent first.</p>
    </div>
...
useHead({ title: 'Talks — Batsaikhan Ookhnoi' })
const api = useApi()
const loading = ref(true)
const items = ref<any[]>([])

onMounted(async () => {
  try { items.value = await api.get<any[]>('/content?type=talk') }
